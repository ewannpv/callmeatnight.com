const populateCarousel = () => {
  for (let index = 0; index < songs.length; index++) {
    ...
    let songLink = songs[index].spotify_link
      ...
      <a 
        href="${songs[index].spotify_link}"
        target="_blank"
        onclick="gtag('event', 'open_song_${songs[index].title}', {'event_category': 'sound_carousel', 'event_label': '${songs[index].title}'});"
      >
        <i class="fa-brands fa-spotify carousel_spotify_logo_available"></i>
      </a>
    </div>
    `
      : `
    <div class="carousel_song_link">
      <i class="fa-brands fa-spotify carousel_spotify_logo_not_available"></i>
    </div>`;

    let songInfo = `
    <div class="carousel_song_title">${songs[index].title}</div>
